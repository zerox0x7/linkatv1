...
class OrderController extends Controller
{
    ...
    public function index(Request $request )
    {
        ...
        $query = Order::where('store_id',$store->id )->with(['user', 'items.orderable', 'items']);
        ...
        $totalSales = Order::where('store_id',$store->id )->whereBetween('created_at', $thisMonth)
            ->where('status', '!=', 'cancelled')
            ->sum('total');
            
        // إجمالي المبيعات للشهر السابق
        $lastMonthSales = Order::where('store_id',$store->id )->whereBetween('created_at', $lastMonth)
            ->where('status', '!=', 'cancelled')
            ->sum('total');
        ...
        $totalOrders = Order::where('store_id',$store->id )->whereBetween('created_at', $thisMonth)->count();
        
        // عدد الطلبات للشهر السابق
        $lastMonthOrders = Order::where('store_id',$store->id )->whereBetween('created_at', $lastMonth)->count();
        ...
        $processingOrders = Order::where('store_id',$store->id )->where('status', 'processing')->count();
            
        $completedOrders = Order::where('store_id',$store->id )->where('status', 'completed')->count();

        $rejectedOrders = Order::where('store_id',$store->id )->where('status', 'rejected')->count();

        $pendingOrders = Order::where('store_id',$store->id )->where('status', 'pending')->count();



        $completedOrdersToday = Order::where('store_id', $store->id)
        ...
        // data for the week
        $processingOrdersThisWeek = Order::where('store_id', $store->id)
        ...
        $pendingOrdersThisWeek = Order::where('store_id', $store->id)
        ...
        $rejectedOrdersThisWeek = Order::where('store_id', $store->id)
...
// إحصائيات المبيعات حسب الشهر للرسم البياني
        ...
        for ($i = 1; $i <= 12; $i++) {
            $monthlySales[$i] = isset($monthlySalesRaw[$i]) ? (float)$monthlySalesRaw[$i] : 0;
                ...
                // Get weekly sales for the past 4 weeks
        $weeklySalesRaw = Order::where('store_id', $store->id)
            ->where('status', 'completed')
            ->whereBetween('created_at', [
                now()->subWeeks(3)->startOfWeek(), // Start of the 4th week from now
                now()->endOfWeek()                // End of the current week
            ])
            ->select(
                DB::raw('YEARWEEK(created_at, 1) as year_week'), // Combine year and week number (ISO-8601 standard, Monday as the first day)
                DB::raw('SUM(total) as total')
            )
            ->groupBy('year_week')
            ->orderBy('year_week')
        ...
        // Build an array for the last 4 weeks
        $weeklySales = [];
        for ($i = 3; $i >= 0; $i--) {
            $weekNumber = now()->subWeeks($i)->format('oW'); // ISO year + week number
            $weeklySales[] = isset($weeklySalesRaw[$weekNumber]) 
                ? (float)$weeklySalesRaw[$weekNumber] 
                : 0; // Total sales for the week or 0 if there's no data
        }
        ...
        return view('themes.admin.orders.index', compact(
            'orders', 
            'customOrderIds', 
            'totalSales', 
            'salesGrowth', 
            ...
            'avgOrderGrowth', 
            'processingOrders',
            'processingOrdersThisWeek',
            'completedOrders',
            'rejectedOrders',
            'rejectedOrdersThisWeek',
            'pendingOrders',
    ...
    public function show(Order $order)
    {
        ...
        return view('themes.admin.orders.show', compact('order', 'digitalCodes', 'hasCustomProducts', 'customProductsData'));
    }
    ...
    public function edit(Order $order)
    {
        ...
        return view('themes.admin.orders.edit', compact('order', 'digitalCodes', 'hasCustomProducts', 'customProductsData'));
    }
