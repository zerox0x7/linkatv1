...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('category')) {
            $query->where('store_id',$store->id )->where('category_id', $request->category);
        }
        
        // فلترة حسب الحالة
        if ($request->filled('status')) {
            $query->where('store_id',$store->id )->where('status', $request->status);
        }
        
        // فلترة حسب البحث
        if ($request->filled('search')) {
            $query-where('store_id',$store->id )->where(function($q) use ($request) {
        ...
        $products = $query->where('store_id',$store->id )->with('category')
                        ->orderBy('created_at', 'desc')
                        ->paginate(15);
        
        $categories = Category::where('store_id',$store->id )->get();
        // dd($products);
        return view('themes.admin.products.index', compact('products', 'categories'));
    }
    ...
    public function create( Request $request )
    {
        $store = $request->attributes->get('store');
        $categories = Category::where('store_id',$store->id )->get();
        return view('themes.admin.products.create', compact('categories'));
    }


    public function advancedCoupon( Request $request )
    {
        $store = $request->attributes->get('store');
        $products = Product::where('store_id',$store->id )->get();

        return view('themes.admin.products.advanced-coupon', compact('products'));
    }
    ...
    {
        return view('themes.admin.products.show', compact('product'));
    }
    ...
    public function edit(Product $product)
    {
        ...
        return view('themes.admin.products.edit', compact('product', 'categories'));
    }
    ...
    public function manageCodes(Product $product)
    {
        ...
        return view('themes.admin.products.manage-codes', compact('product', 'digitalCodes', 'soldCodes'));
    }
    ...
    public function addCode(Request $request, Product $product)
    {
        ...
        if ($existingCode) {
            return redirect()->route('admin.products.manage-codes', $product)
    ...
    {
        return view('themes.admin.products.manage-accounts', compact('product'));
    }
