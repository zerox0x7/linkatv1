...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <button id="cardViewBtn" class="bg-blue-100 text-blue-700 px-3 py-1 rounded shadow-sm text-sm font-medium focus:outline-none">عرض كبطاقات</button>
            <button id="tableViewBtn" class="bg-gray-100 text-gray-700 px-3 py-1 rounded shadow-sm text-sm font-medium focus:outline-none">عرض كجدول</button>
        <a href="{{ route('dashboard.products.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded shadow">
            إضافة منتج جديد
        </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-6">
        <form action="{{ route('dashboard.products.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-end">
                    ...
                    <a href="{{ route('dashboard.products.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white py-2 px-4 rounded shadow">
                        إعادة تعيين
                    </a>
                </div>
            </div>
    ...
    <div id="cardView" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
        @foreach($products as $product)
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-6 border border-gray-100 dark:border-gray-800 flex flex-col justify-between" style="min-height: 440px;">
                <div class="h-48 w-full rounded overflow-hidden bg-gray-100 dark:bg-gray-800 mb-0 flex items-center justify-center relative">
                    @if($product->type == 'digital_card')
                        <span class="absolute top-3 left-3 bg-purple-600 text-white text-sm font-bold px-3 py-1.5 rounded-xl shadow z-10 flex items-center gap-2">
                    ...
                    @elseif($product->type == 'account')
                        <span class="absolute top-3 left-3 bg-blue-600 text-white text-sm font-bold px-3 py-1.5 rounded-xl shadow z-10 flex items-center gap-2">
                    ...
                    @elseif($product->type == 'custom')
                        <span class="absolute top-3 left-3 bg-yellow-600 text-white text-sm font-bold px-3 py-1.5 rounded-xl shadow z-10 flex items-center gap-2">
                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="none" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h8" /></svg>
                            منتج مخصص
                        </span>
                    @endif
                    <img src="{{ asset('storage/' . $product->main_image) }}" class="h-full w-full object-cover rounded-lg transition duration-200" alt="{{ $product->name }}" onerror="this.src='{{ asset('images/product-placeholder.svg') }}';">
                </div>
                <div class="flex-1 flex flex-col justify-between bg-[#1f2937] rounded-b-xl p-4 mt-0" style="margin-top:0;">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        <span class="px-3 py-1 rounded text-sm font-medium bg-gray-800 text-gray-200">{{ $product->category->name ?? 'بدون فئة' }}</span>
                        <span class="px-3 py-1 rounded text-sm font-medium {{ $product->stock > 0 ? 'bg-green-900 text-green-200' : 'bg-red-900 text-red-200' }}">{{ $product->stock > 0 ? 'متوفر' : 'غير متوفر' }}</span>
                        <span class="px-3 py-1 rounded text-sm font-medium {{ $product->status == 'active' ? 'bg-green-800 text-green-100' : ($product->status == 'inactive' ? 'bg-red-800 text-red-100' : 'bg-orange-800 text-orange-100') }}">{{ $product->status == 'active' ? 'نشط' : ($product->status == 'inactive' ? 'غير نشط' : 'نفذ المخزون') }}</span>
                    </div>
                    <div class="mb-3 flex items-center gap-3">
                        @if($product->old_price)
                            <div class="text-lg font-medium text-red-400">{{ number_format($product->price, 2) }} ر.س</div>
                            <div class="text-base text-gray-400 line-through">{{ number_format($product->old_price, 2) }} ر.س</div>
                        @else
                ...
                <div class="flex flex-wrap gap-2 mt-2">
                    @if($product->type == 'digital_card')
                        <a href="{{ route('dashboard.products.manage-codes', $product) }}" class="text-purple-600 dark:text-purple-300 hover:text-purple-900 dark:hover:text-purple-100" title="إدارة الأكواد الرقمية">
                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        </a>
                    @elseif($product->type == 'account')
                        <a href="{{ route('dashboard.products.manage-accounts', $product) }}" class="text-blue-600 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-100" title="إدارة الحسابات">
                    ...
                    <a href="{{ route('dashboard.products.edit', $product) }}" class="text-indigo-600 dark:text-indigo-300 hover:text-indigo-900 dark:hover:text-indigo-100" title="تعديل">
                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </a>
                    <form action="{{ route('dashboard.products.destroy', $product) }}" method="POST" class="inline-block" onsubmit="return confirm('هل أنت متأكد من حذف هذا المنتج؟');">
                        @csrf
                        @method('DELETE')
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex items-center justify-center space-x-3 space-x-reverse">
                                @if($product->type == 'digital_card')
                                    <a href="{{ route('dashboard.products.manage-codes', $product) }}" 
                                ...
                                <a href="{{ route('dashboard.products.edit', $product) }}" class="text-indigo-600 dark:text-indigo-300 hover:text-indigo-900 dark:hover:text-indigo-100">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.product-status-select').forEach(function(select) {
            ...
            select.addEventListener('change', function() {
                ...
                form.method = 'POST';
                form.action = "{{ url('dashboard/dashboard/products') }}/" + productId + "/toggle-status";
