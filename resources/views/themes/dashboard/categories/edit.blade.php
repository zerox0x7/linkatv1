...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">تعديل تصنيف: {{ $category->name }}</h1>
        <p class="text-gray-600 dark:text-gray-300">قم بتعديل معلومات التصنيف</p>
    </div>
        ...
        <form action="{{ route('dashboard.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    @error('parent_id')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-red-500">ملاحظة: لا يمكن جعل التصنيف تابعًا لنفسه أو لأحد فروعه</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    @if($category->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="rounded-lg border w-20 h-20 object-cover" width="100">
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-xs text-gray-500">اترك الحقل فارغًا للاحتفاظ بالصورة الحالية</p>
                    @error('image')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                <div class="flex items-center">
                    <input class="w-4 h-4 text-blue-600 bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 rounded focus:ring-blue-500 focus:ring-2 @error('is_active') border-red-500 @enderror" 
                           type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }}>
            ...
            <div class="mb-6 border-t pt-4">
                ...
                <div class="flex items-center mb-3">
                    <input class="w-4 h-4 text-blue-600 bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 rounded focus:ring-blue-500 focus:ring-2" 
                           type="checkbox" id="show_in_homepage" name="show_in_homepage" value="1" {{ old('show_in_homepage', $category->show_in_homepage) ? 'checked' : '' }}>
                ...
                <div class="mt-3">
                    ...
                    <input type="number" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" 
                           id="homepage_order" name="homepage_order" value="{{ old('homepage_order', $category->homepage_order) }}" min="0">
            ...
            <div class="flex justify-between">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">حفظ التغييرات</button>
            </div>
        </form>
    </div>
@endsection 