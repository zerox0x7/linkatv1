...
<div class="space-y-6">
    ...
    <div id="cardView" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
        @foreach($products as $product)
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 border border-gray-100 dark:border-gray-800 flex flex-col">
                <div class="h-32 w-full rounded overflow-hidden bg-gray-100 dark:bg-gray-800 mb-3 flex items-center justify-center">
                    <img src="{{ asset('storage/' . $product->main_image) }}" class="h-28 object-contain mx-auto" alt="{{ $product->name }}" onerror="this.src='{{ asset('images/product-placeholder.svg') }}'">
                </div>
                <div class="flex-1">
                    <div class="text-base font-bold text-gray-900 dark:text-gray-100 mb-1">{{ $product->name }}</div>
                    ...
                    <div class="flex flex-wrap gap-1 mb-2">
                        ...
                        <span class="px-2 py-0.5 rounded text-xs font-medium {{ $product->status == 'active' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : ($product->status == 'inactive' ? 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' : 'bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200') }}">{{ $product->status == 'active' ? 'نشط' : ($product->status == 'inactive' ? 'غير نشط' : 'نفذ المخزون') }}</span>
                    </div>
                ...
                <div class="flex flex-wrap gap-2 mt-2">
                    ...
                    @elseif($product->type == 'account')
                        <a href="{{ route('admin.products.show', $product) }}#accountsModal" class="text-blue-600 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-100" title="إدارة الحسابات">
                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
