        ...
        <div class="p-6">
            <form action="{{ isset($section) ? route('admin.home-sections.update', $section->id) : route('admin.home-sections.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                   ...
                   <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="max_items" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">الحد الاقصى للمنتجات الظاهرة</label>
                        <input type="number" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-900 dark:text-gray-100 @error('order') border-red-500 dark:border-red-500 @enderror" id="max_items" name="max_items" value="{{ $section->max_items ?? old('order', 0) }}" min="0">
                        <small class="block mt-1 text-xs text-gray-500 dark:text-gray-400">ترتيب ظهور القسم في الصفحة الرئيسية (الأرقام الأصغر تظهر أولاً).</small>
                        @error('order')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
