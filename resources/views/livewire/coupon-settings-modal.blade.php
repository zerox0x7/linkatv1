    ...
    <div class="coupon-content-wrapper">
        ...
        <div class="coupon-page-header text-white" id="pageHeader">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold mb-2 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent rtl-text">تعديل إعدادات الكوبون {{$coupon->code}}</h1>
        ...
        <!-- Main Content -->
        <form wire:submit.prevent="saveCoupons">
            ...
            <div class="coupon-section-card" id="basicSettings">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" value="{{$coupon->used_times}}" readonly class="coupon-input w-full p-4 rounded-xl text-sm opacity-60 cursor-not-allowed" id="usedCount">
                    </div>
                    <div>
            ...
            <!-- Usage Statistics -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8" id="usageStats">
                <div class="coupon-stat-card-1 p-6 rounded-xl">
                    <div class="text-2xl font-bold mb-2 text-[#00e1c0] rtl-text" id="usageCount">{{$coupon->used_times}}</div>
                    <div class="text-sm text-gray-400 rtl-text">مُستخدم</div>
                </div>
                <div class="coupon-stat-card-2 p-6 rounded-xl">
                    <div class="text-2xl font-bold mb-2 text-green-400 rtl-text">
                        @php
                            $savings = 0;
                            if($type === 'percentage') {
                                $savings = ($coupon->used_times * ($minOrderAmount ?? 0) * $value) / 100;
                            } elseif($type === 'fixed') {
                                $savings = $coupon->used_times * $value;
                            }
                        @endphp
                        {{number_format($savings, 0)}} ر.س
                    </div>
                    <div class="text-sm text-gray-400 rtl-text">وفورات</div>
                </div>
                <div class="coupon-stat-card-3 p-6 rounded-xl">
                    <div class="text-2xl font-bold mb-2 text-orange-400 rtl-text" id="usageRate">
                        @php
                            $rate = $maxUses > 0 ? ($coupon->used_times / $maxUses) * 100 : 0;
                        @endphp
                        {{number_format($rate, 1)}}%
                    </div>
                    <div class="text-sm text-gray-400 rtl-text">معدل</div>
                </div>
                <div class="coupon-stat-card-4 p-6 rounded-xl">
                    <div class="text-2xl font-bold mb-2 text-purple-400 rtl-text">
                        @if($coupon->expires_at)
                            @php
                                $remaining = \Carbon\Carbon::parse($coupon->expires_at)->diff(now());
                                $remainingText = '';
                                if($remaining->days > 0) {
                                    $remainingText = $remaining->days . 'ي ';
                                }
                                $remainingText .= $remaining->h . 'س';
                            @endphp
                            {{$remainingText}}
                        @else
                            غير محدد
                    ...
                    <div class="text-sm text-gray-400 rtl-text">متبقي</div>
                </div>
            </div>

            <!-- Action Buttons -->
            <div class="flex flex-col sm:flex-row gap-4" id="actionButtons">
                <button type="submit" class="coupon-btn-primary flex-1 px-8 py-4 font-semibold rounded-xl transition-all rtl-text text-lg" onclick="console.log('Save button clicked');">
                    <span wire:loading.remove wire:target="saveCoupons">حفظ الإعدادات</span>
                    <span wire:loading wire:target="saveCoupons">جاري الحفظ...</span>
                </button>
                <button type="button" wire:click="testSave" class="coupon-btn-secondary flex-1 px-8 py-4 font-medium rounded-xl transition-all rtl-text text-lg">اختبار</button>
...
<div id="couponWizardOverlay" class="coupon-wizard-overlay">
    ...
    <div id="couponWizardTooltip" class="coupon-wizard-tooltip">
        ...
        <p class="coupon-wizard-description" id="wizardDescription">
            دعنا نأخذ جولة سريعة في صفحة إعدادات الكوبون لمساعدتك في فهم جميع الميزات المتاحة.
        </p>
...
<script>
    ...
    const wizardSteps = [
        {
            target: 'pageHeader',
            title: 'مرحباً بك في صفحة تعديل الكوبون',
            description: 'تتيح لك هذه الصفحة تكوين جميع جوانب كوبونات التخفيض السريع الخاصة بك. دعنا نستكشف كل قسم لمساعدتك في الحصول على أقصى استفادة من حملات الكوبون الخاصة بك.',
            position: 'bottom'
        },
        {
