...
use App\Http\Controllers\Dashboard\HomeSectionSettingController;
...
use App\Http\Controllers\Dashboard\OnlineUsersController;
...
|--------------------------------------------------------------------------
|
| U+U+O USOU. OO1OUSU? OU.USO1 U.O3OOOO U,U^O-Oc OU,U.O3OU^U,OO U^OUU^U+ OO-O OU,U.OU.U^O1Oc /Dashboard
|
*/
...
Route::prefix('dashboard')->middleware(['auth','customer'])->group(function () {
    ...
    Route::patch('reviews/{review}/toggle-approval', [\App\Http\Controllers\Dashboard\ReviewController::class, 'toggleApproval'])->name('reviews.toggle-approval');
    
    // إدارة البطاقات الرقمية
    Route::resource('digital-cards', \App\Http\Controllers\Dashboard\DigitalCardController::class);
    
    // OO_OOOc OU,OU,O"OO
    Route::resource('orders', OrderController::class)->except(['create', 'store']);
    Route::put('orders/{order}/status', [OrderController::class, 'updateStatus'])->name('orders.status');
    Route::post('orders/{order}/whatsapp-alert', [App\Http\Controllers\Dashboard\WhatsAppController::class, 'sendOrderAlert'])->name('orders.whatsapp_alert');
    
    // OO_OOOc OU,U.O3OOrO_U.USU+
    Route::resource('users', UserController::class);
    
    // OU,OU,O"OO OU,U.OrOOOc
    Route::redirect('custom-orders', '/Dashboard/custom_orders')->name('custom-orders.redirect');
    Route::resource('custom_orders', \App\Http\Controllers\Dashboard\CustomOrderController::class);
    Route::post('custom_orders/{custom_order}/reply', [\App\Http\Controllers\Dashboard\CustomOrderController::class, 'reply'])->name('custom_orders.reply');
    Route::put('custom_orders/{custom_order}/status', [\App\Http\Controllers\Dashboard\CustomOrderController::class, 'updateStatus'])->name('custom_orders.update_status');
    
    // 
    Route::get('marquee', [\App\Http\Controllers\Dashboard\MarqueeController::class, 'index'])->name('marquee');
    Route::post('marquee/save', [\App\Http\Controllers\Dashboard\MarqueeController::class, 'save'])->name('marquee.save');
    Route::post('marquee/update-item', [\App\Http\Controllers\Dashboard\MarqueeController::class, 'updateItem'])->name('marquee.update-item');
    Route::post('marquee/toggle-item', [\App\Http\Controllers\Dashboard\MarqueeController::class, 'toggleItem'])->name('marquee.toggle-item');
    Route::post('marquee/delete-item', [\App\Http\Controllers\Dashboard\MarqueeController::class, 'deleteItem'])->name('marquee.delete-item');
    Route::get('settings', [SettingController::class, 'index'])->name('settings.index');
    Route::put('settings', [SettingController::class, 'update'])->name('settings.update');
    Route::post('settings/clear-cache', [SettingController::class, 'clearCache'])->name('settings.clear-cache');
    Route::post('settings/payment-icons/upload', [SettingController::class, 'uploadPaymentIcons'])->name('Dashboard.settings.payment-icons.upload');
    Route::post('settings/payment-icons/delete', [SettingController::class, 'deletePaymentIcon'])->name('Dashboard.settings.payment-icons.delete');
    
    // إعدادات الأكواد المخصصة (CSS/JS)
    Route::get('settings/custom-code', [\App\Http\Controllers\Dashboard\SettingController::class, 'customCode'])->name('Dashboard.settings.custom_code');
    Route::post('settings/custom-code', [\App\Http\Controllers\Dashboard\SettingController::class, 'saveCustomCode'])->name('Dashboard.settings.save_custom_code');
    ...
    Route::get('reports/users', [\App\Http\Controllers\Dashboard\ReportController::class, 'users'])->name('reports.users');
    
    // OO_OOOc OU,OU?O-OO OU,OOO"OOc
    Route::resource('pages', \App\Http\Controllers\Dashboard\PageController::class);
    Route::get('home-manager', [\App\Http\Controllers\Dashboard\PageController::class, 'homeManager'])->name('pages.home-manager');
    
    // OO_OOOc OU^OO"O OU,U?U^OO
    Route::resource('menu-links', \App\Http\Controllers\Dashboard\MenuLinkController::class);
    ...
    Route::get('online-users', [\App\Http\Controllers\Dashboard\OnlineUsersController::class, 'index'])->name('online-users.index');
    Route::post('online-users/update-activity', [\App\Http\Controllers\Dashboard\OnlineUsersController::class, 'updateActivity'])->name('online-users.update-activity');
    Route::post('online-users/clear-inactive', [\App\Http\Controllers\Dashboard\OnlineUsersController::class, 'clearInactive'])->name('online-users.clear-inactive');
}); 
